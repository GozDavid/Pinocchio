...
int allocate_main_memory()
{
  ...
  */
  ...
  h_gpu_products = (gpu_product_data *)malloc(MyGrids[0].total_local_size * sizeof(gpu_product_data));
  gpu_products   = (gpu_product_data *)omp_target_alloc((MyGrids[0].total_local_size * sizeof(gpu_product_data)), internal.devID);

  assert((h_gpu_products != NULL) && (gpu_products != NULL));
  ...
  /* allocate gpu_second_derivatives on the GPU */
  gpu_second_derivatives = (double **)omp_target_alloc((6 * sizeof(double *)), internal.devID);
  assert(gpu_second_derivatives != NULL);
  for (int i=0 ; i<6 ; i++)
    {
      gpu_second_derivatives[i] = (double *)omp_target_alloc(MyGrids[0].total_local_size * sizeof(double), internal.devID);
      assert(gpu_second_derivatives[i] != NULL);
    }
