...
void common_initialization(const unsigned int size)
{
  ...
  #pragma omp target teams distribute            \
                     parallel for                \
                     is_device_ptr(gpu_products) \
                     device(internal.devID)
  for (unsigned int i=0 ; i<size ; i++)
    {
      gpu_products[i].Rmax = (PRODFLOAT)-1;
      gpu_products[i].Fmax = (PRODFLOAT)-10.0;
    }
...
int compute_collapse_times(int ismooth)
{
  ...
  /* copy second_derivatives to the GPU (synchronously) */
  for (int i=0 ; i<6 ; i++)
    omp_target_memcpy(gpu_second_derivatives[i], second_derivatives[0][i], (total_size * sizeof(double)), 0, 0, internal.devID, internal.hostID);
...
#pragma omp target teams distribute parallel for reduction(+: local_average, local_variance, all_fails) \
                   is_device_ptr(gpu_second_derivatives, gpu_products)                                  \
                   device(internal.devID)
...
#pragma omp parallel for reduction(+: local_average, local_variance, all_fails)
  ...
  for (unsigned int index=0 ; index<total_size ; index++)
      ...
      for (int i=0 ; i<6 ; i++)
	{
	  diff_ten[i] =
#ifdef GPU_OMP
                        gpu_second_derivatives[i][index];
#else
	                second_derivatives[0][i][index];
#endif // GPU_OMP
	}
      ...
      const int update = (gpu_products[index].Fmax < Fnew);
      gpu_products[index].Rmax = (update ? ismooth : gpu_products[index].Rmax);
      gpu_products[index].Fmax = (update ? Fnew    : gpu_products[index].Fmax);
  ...
  /* copy products back to the GPU (synchronously) */
  omp_target_memcpy(h_gpu_products, gpu_products, (total_size * sizeof(gpu_product_data)), 0, 0, internal.hostID, internal.devID);
  ...
  for (unsigned int index=0 ; index<total_size ; index++)
    {
      products[index].Rmax = h_gpu_products[index].Rmax;
      products[index].Fmax = h_gpu_products[index].Fmax;
    }
