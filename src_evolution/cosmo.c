...
int initialize_cosmology()
{
  ...
  */
  ...
  /* host gpu_spline allocation */
  h_gpu_spline = custom_cubic_spline_alloc(NBINS);
  /* host gou_spline initialization */
  custom_cubic_spline_init(h_gpu_spline, grow1, scalef, NBINS);

  // Allocate memory on the GPU for gpu_spline and its components
  gpu_spline = (CubicSpline *)omp_target_alloc(sizeof(CubicSpline), internal.devID);
  assert(gpu_spline != NULL);

  gpu_spline->x        = (double *)omp_target_alloc(h_gpu_spline->size       * sizeof(double), internal.devID);
  gpu_spline->y        = (double *)omp_target_alloc(h_gpu_spline->size       * sizeof(double), internal.devID);
  gpu_spline->d2y_data = (double *)omp_target_alloc((h_gpu_spline->size - 1) * sizeof(double), internal.devID);
  gpu_spline->coeff_a  = (double *)omp_target_alloc((h_gpu_spline->size - 1) * sizeof(double), internal.devID);
  gpu_spline->coeff_b  = (double *)omp_target_alloc((h_gpu_spline->size - 1) * sizeof(double), internal.devID);
  gpu_spline->coeff_c  = (double *)omp_target_alloc((h_gpu_spline->size - 1) * sizeof(double), internal.devID);
  gpu_spline->coeff_d  = (double *)omp_target_alloc((h_gpu_spline->size - 1) * sizeof(double), internal.devID);

  assert((gpu_spline->x        != NULL) &&
	 (gpu_spline->y        != NULL) &&
	 (gpu_spline->d2y_data != NULL) &&
	 (gpu_spline->coeff_a  != NULL) &&
	 (gpu_spline->coeff_b  != NULL) &&
	 (gpu_spline->coeff_c  != NULL) &&
	 (gpu_spline->coeff_d  != NULL));
  
  /* copy the spline to the GPU */
  double cputmp = MPI_Wtime();  

  omp_target_memcpy(&gpu_spline->size,    &h_gpu_spline->size,    sizeof(int),                                 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->x,        h_gpu_spline->x,        (h_gpu_spline->size       * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->y,        h_gpu_spline->y,        (h_gpu_spline->size       * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->d2y_data, h_gpu_spline->d2y_data, ((h_gpu_spline->size - 1) * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->coeff_a,  h_gpu_spline->coeff_a,  ((h_gpu_spline->size - 1) * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->coeff_b,  h_gpu_spline->coeff_b,  ((h_gpu_spline->size - 1) * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->coeff_c,  h_gpu_spline->coeff_c,  ((h_gpu_spline->size - 1) * sizeof(double)), 0, 0, internal.devID, internal.hostID);
  omp_target_memcpy(gpu_spline->coeff_d,  h_gpu_spline->coeff_d,  ((h_gpu_spline->size - 1) * sizeof(double)), 0, 0, internal.devID, internal.hostID);
			  ...
			  const int    ismooth)
{
  ...
  return 1./pow(10.,my_spline_eval(SPLINE_INVGROW[ismooth], log10(D), ACCEL_INVGROW[ismooth])) -1.;
     ...
     return 1./pow(10.0, custom_cubic_spline_eval(gpu_spline, log10(D))) -1;
