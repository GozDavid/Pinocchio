...
int compute_fmax(void)
{
  ...
  */
  ...
  /*---------------- Free GPU memory and host_gpu buffers */

  /*----- Free GPU spline ----*/
  omp_target_free(gpu_spline->x,        internal.devID);
  omp_target_free(gpu_spline->y,        internal.devID);
  omp_target_free(gpu_spline->d2y_data, internal.devID);
  omp_target_free(gpu_spline->coeff_a,  internal.devID);
  omp_target_free(gpu_spline->coeff_b,  internal.devID);
  omp_target_free(gpu_spline->coeff_c,  internal.devID);
  omp_target_free(gpu_spline->coeff_d,  internal.devID);
  omp_target_free(gpu_spline,           internal.devID);
  custom_cubic_spline_free(h_gpu_spline);

  /*---- Free products -----*/
  omp_target_free(gpu_products, internal.devID);
  free(h_gpu_products);

  /*---- Free second derivatives ----*/
  for (int i=5 ; i<=0 ; i--)
    omp_target_free(gpu_second_derivatives[i], internal.devID);
